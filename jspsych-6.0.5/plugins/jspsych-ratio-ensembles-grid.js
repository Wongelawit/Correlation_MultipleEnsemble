...
jsPsych.plugins['ratio-ensembles-grid'] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('ratio-ensembles-grid', 'stimuli', 'image');

  plugin.info = {
    name: 'ratio-ensembles-grid',
    description: '',
    parameters: {
      stimuli: {
	  ...
	  population_ratio: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Population Ratio',
        default: 1,
        description: 'Ratio determining population sizes. '
      },
      choices: {
	  ...
	  mask: {
        type: jsPsych.plugins.parameterType.BOOL,
        pretty_name: 'Is this trial a mask',
        default: false,
        description: 'If true, data recorded will match mask requirements.'
      },
	  prev_trial: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Previous trial correct key',
        default: null,
        description: 'If not null, the correct key for preceding stimulus.'
      }
    }
  }
  ...
  plugin.trial = function(display_element, trial) {
	...
	populations = generate_sizes(trial.population_ratio);
	
    var new_html = plugin.generate_stimulus(trial.stimuli, trial.grid_size, populations);
    ...
    var end_trial = function() {
      ...
      if (trial.mask != true) {
		var trial_data = {
		  ...
		  "target_size": populations[0],
		  "distractor_size": populations[1],
		  "ratio": trial.population_ratio,
		  "actual_population_ratio": populations[0]/populations[1]
        };
	  } else {
		var trial_data = {
          "rt": response.rt,
          "key_press": response.key,
		  "correct_key": trial.prev_trial,
        };
	  }
  ...
  function generate_coordinates(row, col, size, excluding_coordinates) {
	...
	/**
   * Generates two population sizes which sum to [30,200] based on given ratio.
   *
   * @param  ratio                   {int}
   *         
   *
   * @return population_sizes          [target, distractor]      
   */ 
  ...
  function generate_sizes(ratio) {
	...
	if (ratio < 0) {
		max = Math.floor(200 / ((ratio * -1) + 1));
		min = Math.ceil(30 / ((ratio * -1) + 1));
		distractor_num = Math.floor(Math.random() * (max - min + 1) + min);
		target_num = Math.floor(distractor_num * ratio * -1);
	} else {
		max = (200 / (ratio + 1));
		min = (30 / (ratio + 1));
		target_num = Math.floor(Math.random() * (max - min + 1) + min);
		distractor_num = Math.floor(target_num * ratio);
	}
